...
#
# 使い方:
#   bash install.sh -d /dev/vda              # VM (GPU なし, デフォルト)
#   bash install.sh -d /dev/vda -t nixos-vm  # VM (nixos-vm ターゲット)
#   bash install.sh -d /dev/nvme0n1 -g nvidia -b 1GiB -r 57GiB  # 物理マシン
...
NIX_GPU="${NIX_GPU:-default}"
NIX_CPU="${NIX_CPU:-x86_64-linux}"
...
# ── 引数パース ─────────────────────────────────────────────────
while getopts "d:g:c:b:r:t:h" opt; do
  case "$opt" in
    d) DEVICE="$OPTARG" ;;
    g) NIX_GPU="$OPTARG" ;;
    c) NIX_CPU="$OPTARG" ;;
...
echo -e "  NIX_CPU          : ${NIX_CPU}"
echo -e "  NIX_GPU          : ${NIX_GPU}"
...
echo
info "nixos-install 開始: NIX_CPU=${NIX_CPU} NIX_GPU=${NIX_GPU} → .#${FLAKE_TARGET}"
NIX_CPU="$NIX_CPU" NIX_GPU="$NIX_GPU" \
  nixos-install --impure --flake "${MOUNT_ROOT}/etc/nixos#${FLAKE_TARGET}"
