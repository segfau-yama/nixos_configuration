...
### ハードウェア設定
GPU と CPU の種類は `modules/hosts/` 内の設定ファイルで**宣言的**に指定します。
環境変数や `--impure` フラグは不要です。

| ホスト | ファイル | 用途 |
|---|---|---|
| `nixos` | `modules/hosts/base/configuration.nix` | 物理マシン（メインデスクトップ） |
| `nixos-vm` | `modules/hosts/vm/configuration.nix` | 仮想マシン（テスト用） |
...
`hardware` モジュール（`modules/hardware/hardware.nix`）が宣言値に応じて
ドライバー・マイクロコード・btrfs-progs を自動適用します。

ハードウェアを変更する場合は該当ファイルを編集して `nixos-rebuild switch` で反映します。
...
# 物理マシン (NVMe)
bash install.sh -d /dev/nvme0n1 -b 1GiB -r 57GiB
```
...
> **注意**: インストール先のハードウェアが `modules/hosts/base/configuration.nix` の
> `my.hardware.*` 設定と一致していることを確認してください。
> 異なる場合はインストール前にファイルを編集してください。
...
> **GPU 設定**: `nixos-vm` ターゲットは `my.hardware.gpu = "none"` で構成されているため、
> GPU パススルーなしの通常の VM に適しています。
...
# 仮想マシン
nixos-rebuild switch --flake /etc/nixos#nixos-vm
```
...
- **ハードウェア設定**: `modules/hardware/hardware.nix`
  - GPU / CPU の種類は `modules/hosts/base/configuration.nix` の `my.hardware.*` で指定
- **システム全体**: `modules/system/` 配下
  - 例: `modules/system/settings/audio/audio.nix`（音声設定）
- **アプリ・ツール**: `modules/programs/` 配下
  - 例: `modules/programs/gaming/gaming.nix`（Lutris / Wine）
  - 例: `modules/programs/programming/programming.nix`（エディター / 言語ツール）
  - 例: `modules/programs/niri/niri.nix`（Niri キーバインド）
- **ユーザー設定**: `modules/users/jade/jade.nix`
