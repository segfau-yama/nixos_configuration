...
# aarch64 + NVIDIA
NIX_CPU=aarch64-linux NIX_GPU=nvidia sudo nixos-install --flake .#nixos

# x86_64 + AMD
NIX_GPU=amd sudo nixos-install --flake .#nixos

# aarch64 + GPU なし
NIX_CPU=aarch64-linux NIX_GPU=default sudo nixos-install --flake .#nixos
```
...
既存 NixOS でこの設定を使う場合、インストール時の環境変数と同じ値を指定します：
	...
	# aarch64 + NVIDIA
	NIX_CPU=aarch64-linux NIX_GPU=nvidia sudo nixos-rebuild switch --flake /etc/nixos#nixos

	# x86_64 + AMD
	NIX_GPU=amd sudo nixos-rebuild switch --flake /etc/nixos#nixos

	# aarch64 + GPU なし
	NIX_CPU=aarch64-linux NIX_GPU=default sudo nixos-rebuild switch --flake /etc/nixos#nixos
